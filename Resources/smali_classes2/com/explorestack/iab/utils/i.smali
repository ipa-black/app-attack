.class public Lcom/explorestack/iab/utils/i;
.super Lcom/explorestack/iab/utils/g;
.source "SourceFile"


# annotations
.annotation system Ldalvik/annotation/Signature;
    value = {
        "Lcom/explorestack/iab/utils/g<",
        "Lcom/explorestack/iab/vast/view/CircleCountdownView;",
        ">;"
    }
.end annotation


# instance fields
.field public g:Z


# direct methods
.method public constructor <init>(Landroid/view/View$OnClickListener;)V
    .locals 0

    invoke-direct {p0, p1}, Lcom/explorestack/iab/utils/g;-><init>(Landroid/view/View$OnClickListener;)V

    const/4 p1, 0x0

    iput-boolean p1, p0, Lcom/explorestack/iab/utils/i;->g:Z

    return-void
.end method


# virtual methods
.method public bridge synthetic a(Landroid/content/Context;Landroid/view/View;Lcom/explorestack/iab/utils/IabElementStyle;)V
    .locals 0

    check-cast p2, Lcom/explorestack/iab/vast/view/CircleCountdownView;

    invoke-virtual {p0, p1, p2, p3}, Lcom/explorestack/iab/utils/i;->a(Landroid/content/Context;Lcom/explorestack/iab/vast/view/CircleCountdownView;Lcom/explorestack/iab/utils/IabElementStyle;)V

    return-void
.end method

.method public a(Landroid/content/Context;Lcom/explorestack/iab/vast/view/CircleCountdownView;Lcom/explorestack/iab/utils/IabElementStyle;)V
    .locals 0

    iget-boolean p1, p0, Lcom/explorestack/iab/utils/i;->g:Z

    if-eqz p1, :cond_0

    const-string p1, ""

    goto :goto_0

    :cond_0
    const-string p1, ""

    :goto_0
    invoke-static {p1}, Lcom/explorestack/iab/utils/Assets;->getBitmapFromBase64(Ljava/lang/String;)Landroid/graphics/Bitmap;

    move-result-object p1

    invoke-virtual {p2, p1}, Lcom/explorestack/iab/vast/view/CircleCountdownView;->setImage(Landroid/graphics/Bitmap;)V

    return-void
.end method

.method public a(Z)V
    .locals 0

    iput-boolean p1, p0, Lcom/explorestack/iab/utils/i;->g:Z

    invoke-virtual {p0}, Lcom/explorestack/iab/utils/g;->d()V

    return-void
.end method

.method public bridge synthetic b(Landroid/content/Context;Lcom/explorestack/iab/utils/IabElementStyle;)Landroid/view/View;
    .locals 0

    invoke-virtual {p0, p1, p2}, Lcom/explorestack/iab/utils/i;->d(Landroid/content/Context;Lcom/explorestack/iab/utils/IabElementStyle;)Lcom/explorestack/iab/vast/view/CircleCountdownView;

    move-result-object p1

    return-object p1
.end method

.method public c(Landroid/content/Context;Lcom/explorestack/iab/utils/IabElementStyle;)Lcom/explorestack/iab/utils/IabElementStyle;
    .locals 0

    if-eqz p2, :cond_0

    invoke-virtual {p2}, Lcom/explorestack/iab/utils/IabElementStyle;->getStyle()Ljava/lang/String;

    move-result-object p1

    const-string p2, "speakerfill"

    invoke-virtual {p2, p1}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result p1

    if-eqz p1, :cond_0

    new-instance p1, Lcom/explorestack/iab/utils/IabElementStyle;

    invoke-direct {p1}, Lcom/explorestack/iab/utils/IabElementStyle;-><init>()V

    sget-object p2, Ljava/lang/Boolean;->TRUE:Ljava/lang/Boolean;

    invoke-virtual {p1, p2}, Lcom/explorestack/iab/utils/IabElementStyle;->setOutlined(Ljava/lang/Boolean;)V

    sget-object p2, Lcom/explorestack/iab/utils/Assets;->defMuteStyle:Lcom/explorestack/iab/utils/IabElementStyle;

    invoke-virtual {p2, p1}, Lcom/explorestack/iab/utils/IabElementStyle;->copyWith(Lcom/explorestack/iab/utils/IabElementStyle;)Lcom/explorestack/iab/utils/IabElementStyle;

    move-result-object p1

    return-object p1

    :cond_0
    sget-object p1, Lcom/explorestack/iab/utils/Assets;->defMuteStyle:Lcom/explorestack/iab/utils/IabElementStyle;

    return-object p1
.end method

.method public d(Landroid/content/Context;Lcom/explorestack/iab/utils/IabElementStyle;)Lcom/explorestack/iab/vast/view/CircleCountdownView;
    .locals 0

    new-instance p2, Lcom/explorestack/iab/vast/view/CircleCountdownView;

    invoke-direct {p2, p1}, Lcom/explorestack/iab/vast/view/CircleCountdownView;-><init>(Landroid/content/Context;)V

    return-object p2
.end method
