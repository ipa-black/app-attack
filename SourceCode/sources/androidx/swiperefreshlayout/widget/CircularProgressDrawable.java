...
public class CircularProgressDrawable extends Drawable implements Animatable {
    ...
    private void applyFinishTranslation(float f2, Ring ring) {
        ...
        ring.setRotation(ring.getStartingRotation() + ((((float) (Math.floor(ring.getStartingRotation() / MAX_PROGRESS_ARC) + 1.0d)) - ring.getStartingRotation()) * f2));
    }
    ...
    void applyTransformation(float f2, Ring ring, boolean z) {
        ...
        if (this.mFinishing) {
            ...
            if (f2 < SHRINK_OFFSET) {
                float f4 = f2 / SHRINK_OFFSET;
                interpolation = ring.getStartingStartTrim();
                f3 = (MATERIAL_INTERPOLATOR.getInterpolation(f4) * 0.79f) + MIN_PROGRESS_ARC + interpolation;
            } else {
                float f5 = (f2 - SHRINK_OFFSET) / SHRINK_OFFSET;
                float startingStartTrim = ring.getStartingStartTrim() + 0.79f;
                interpolation = startingStartTrim - (((1.0f - MATERIAL_INTERPOLATOR.getInterpolation(f5)) * 0.79f) + MIN_PROGRESS_ARC);
                f3 = startingStartTrim;
            }
            float f6 = startingRotation + (RING_ROTATION * f2);
            float f7 = (f2 + this.mRotationCount) * GROUP_FULL_ROTATION;
