...
public class PackageManagerHelper {
    ...
    public static void setComponentEnabled(Context context, Class<?> klazz, boolean enabled) {
        try {
            ...
            Logger.get().debug(TAG, String.format("%s %s", klazz.getName(), enabled ? "enabled" : LogConstants.MSG_AD_TYPE_DISABLED), new Throwable[0]);
        } catch (Exception e2) {
            Logger.get().debug(TAG, String.format("%s could not be %s", klazz.getName(), enabled ? "enabled" : LogConstants.MSG_AD_TYPE_DISABLED), e2);
        }
    }
