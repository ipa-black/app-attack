...
public class HyperSpline {
    ...
    public void getVelocity(double d2, double[] dArr) {
        ...
        for (int i2 = 0; i2 < dArr.length; i2++) {
            dArr[i2] = this.mCurve[i2][i].vel(d3 / this.mCurveLength[i]);
        }
    }
    ...
    public void getPos(double d2, double[] dArr) {
        ...
        for (int i2 = 0; i2 < dArr.length; i2++) {
            dArr[i2] = this.mCurve[i2][i].eval(d3 / this.mCurveLength[i]);
        }
    }
    ...
    public void getPos(double d2, float[] fArr) {
        ...
        for (int i2 = 0; i2 < fArr.length; i2++) {
            fArr[i2] = (float) this.mCurve[i2][i].eval(d3 / this.mCurveLength[i]);
        }
    }
    ...
    public double getPos(double d2, int i) {
        ...
        return this.mCurve[i][i2].eval(d3 / dArr[i2]);
    }
    ...
    static Cubic[] calcNaturalCubic(int i, double[] dArr) {
        ...
        double d2 = (((dArr[i2] - dArr[i6]) * 3.0d) - dArr3[i6]) * dArr2[i2];
        dArr3[i2] = d2;
        dArr4[i2] = d2;
        while (i6 >= 0) {
        ...
        while (i3 < i2) {
            ...
            double d5 = dArr[i9];
            double d6 = dArr4[i9];
            cubicArr[i3] = new Cubic((float) d3, d4, (((d5 - d3) * 3.0d) - (d4 * 2.0d)) - d6, ((d3 - d5) * 2.0d) + d4 + d6);
            i3 = i9;
        }
        return cubicArr;
    }
    ...
    public static class Cubic {
        ...
        public double vel(double d2) {
            return (((this.mD * 3.0d * d2) + (this.mC * 2.0d)) * d2) + this.mB;
        }
    }
}
