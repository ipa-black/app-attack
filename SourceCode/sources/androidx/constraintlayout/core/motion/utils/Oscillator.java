...
public class Oscillator {
    ...
    public void normalize() {
        ...
        while (true) {
            ...
            double[] dArr = this.mPosition;
            d3 += (dArr[i3] - dArr[i3 - 1]) * ((fArr2[i] + fArr2[i3]) / 2.0f);
            i3++;
        }
        int i4 = 0;
        while (true) {
        ...
        while (true) {
            if (i5 < this.mPeriod.length) {
                ...
                dArr3[i5] = dArr3[i6] + (d4 * ((fArr3[i6] + fArr3[i5]) / 2.0f));
                i5++;
            } else {
    ...
    double getP(double d2) {
        ...
        if (binarySearch != 0) {
            ...
            return this.mArea[i3] + ((f3 - (d5 * d4)) * (d2 - d4)) + ((d5 * ((d2 * d2) - (d4 * d4))) / 2.0d);
        }
        return 0.0d;
    }
