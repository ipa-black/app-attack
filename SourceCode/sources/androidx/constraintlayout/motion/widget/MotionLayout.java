    ...
    public void setTransition(int transitionId) {
        ...
        if (this.mScene != null) {
            ...
            if (i == this.mBeginState) {
                ...
                f2 = i == this.mEndState ? 1.0f : Float.NaN;
            }
    ...
    public void setupMotionViews() {
        ...
        if (staggered != 0.0f) {
            ...
            while (i < childCount) {
                ...
                motionController10.mStaggerOffset = abs - (((f7 - f5) * abs) / (f4 - f5));
                i++;
            }
        }
    }
    ...
    */
    ...
    class DecelerateInterpolator extends MotionInterpolator {
        ...
        public float getInterpolation(float time) {
            ...
            if (f4 > 0.0f) {
                ...
                f2 = (this.initalV * time) - (((this.maxA * time) * time) / 2.0f);
                f3 = this.currentP;
            } else {
                float f6 = this.maxA;
                if ((-f4) / f6 < time) {
                ...
                MotionLayout.this.mLastVelocity = f4 + (f6 * time);
                f2 = (this.initalV * time) + (((this.maxA * time) * time) / 2.0f);
                f3 = this.currentP;
            }
            return f2 + f3;
    ...
    public void getViewVelocity(View view, float posOnViewX, float posOnViewY, float[] returnVelocity, int type) {
        ...
        if (this.mInterpolator != null) {
            ...
            f3 = (signum * ((interpolation - f2) / EPSILON)) / this.mTransitionDuration;
        } else {
            f2 = f4;
        }
    ...
    public class Model {
        ...
        private void debugWidget(String str, ConstraintWidget child) {
            ...
            Log.v(MotionLayout.TAG, str + append3.append(str5).toString() + " ---  " + child);
        }

        private void debugLayout(String title, ConstraintWidgetContainer c2) {
            String str = title + " " + Debug.getName((View) c2.getCompanionWidget());
            Log.v(MotionLayout.TAG, str + "  ========= " + c2);
            int size = c2.getChildren().size();
            for (int i = 0; i < size; i++) {
                String str2 = str + "[" + i + "] ";
                ConstraintWidget constraintWidget = c2.getChildren().get(i);
    ...
    public void onNestedPreScroll(final View target, int dx, int dy, int[] consumed, int type) {
        ...
        if (!transition.isEnabled() || (touchResponse = transition.getTouchResponse()) == null || (touchRegionId = touchResponse.getTouchRegionId()) == -1 || target.getId() == touchRegionId) {
            if (motionScene.getMoveWhenScrollAtTop()) {
                ...
                if ((f2 == 1.0f || f2 == 0.0f) && target.canScrollVertically(i)) {
                    return;
                }
            }
    ...
    private class DevModeDraw {
                ...
                if (mode != 4 || this.mPathMode[i3 - 1] != 0) {
                    ...
                    if (mode == 4) {
                        ...
                        if (i6 == 1) {
                            ...
                            f2 = f5;
                            f3 = f4;
                            drawPathScreenTicks(canvas, f4 - 0.0f, f5 - 0.0f, i, i2);
                    ...
                    if (mode == 6) {
                        drawPathScreenTicks(canvas, f3 - 0.0f, f2 - 0.0f, i, i2);
                    }
                    canvas.drawPath(this.mPath, this.mFillPaint);
                }
            }
        ...
        private void drawPathCartesianTicks(Canvas canvas, float x, float y) {
            ...
            String str = "" + (((int) (((min2 * 100.0f) / Math.abs(f4 - f2)) + 0.5d)) / 100.0f);
            getTextBounds(str, this.mTextPaint);
            ...
            String str2 = "" + (((int) (((max2 * 100.0f) / Math.abs(f5 - f3)) + 0.5d)) / 100.0f);
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if ((this.mDebugPath & 1) == 1 && !isInEditMode()) {
            ...
            int i = this.mCurrentState;
            String sb = append.append(i == -1 ? AdError.UNDEFINED_DOMAIN : Debug.getState(this, i)).toString();
