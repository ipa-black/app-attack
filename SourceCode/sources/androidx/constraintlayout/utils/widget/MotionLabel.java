...
public class MotionLabel extends View implements FloatLayout {
    ...
    private void adjustTexture(float l, float t, float r, float b2) {
        if (this.mTextShaderMatrix == null) {
            return;
        }
        this.mFloatWidth = r - l;
        this.mFloatHeight = b2 - t;
        updateShaderMatrix();
    }
    ...
    private float getVerticalOffset() {
        ...
        return ((((((Float.isNaN(this.mFloatHeight) ? getMeasuredHeight() : this.mFloatHeight) - getPaddingTop()) - getPaddingBottom()) - ((fontMetrics.descent - fontMetrics.ascent) * f2)) * (1.0f - this.mTextPanY)) / 2.0f) - (f2 * fontMetrics.ascent);
    }
    ...
    public void layout(int l, int t, int r, int b2) {
        ...
        if (this.mUseOutline || !isNaN) {
            adjustTexture(l, t, r, b2);
            buildShape(f2);
        }
    }

    @Override // androidx.constraintlayout.motion.widget.FloatLayout
    public void layout(float l, float t, float r, float b2) {
        int i = (int) (l + 0.5f);
        this.mDeltaLeft = l - i;
        int i2 = (int) (r + 0.5f);
        int i3 = i2 - i;
        ...
        if (this.mAutoSize) {
            ...
            if (width * f5 > height * f4) {
                this.mPaint.setTextSize((this.paintTextSize * f4) / width);
            } else {
                this.mPaint.setTextSize((this.paintTextSize * f5) / height);
            }
