...
public class ArrayIterator implements ResettableIterator {
    ...
    protected void checkBound(int i, String str) {
        if (i > this.endIndex) {
            throw new ArrayIndexOutOfBoundsException(new StringBuffer("Attempt to make an ArrayIterator that ").append(str).append("s beyond the end of the array. ").toString());
        }
        if (i < 0) {
            throw new ArrayIndexOutOfBoundsException(new StringBuffer("Attempt to make an ArrayIterator that ").append(str).append("s before the start of the array. ").toString());
        }
    }
