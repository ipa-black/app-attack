...
public final class EndianUtils {
    ...
    public static long readSwappedLong(byte[] bArr, int i) {
        return (bArr[i] & 255) + ((bArr[i + 1] & 255) << 8) + ((bArr[i + 2] & 255) << 16) + ((bArr[i + 3] & 255) << 24) + (((((bArr[i + 4] & 255) + ((bArr[i + 5] & 255) << 8)) + ((bArr[i + 6] & 255) << 16)) + ((bArr[i + 7] & 255) << 24)) << 32);
    }

    public static void writeSwappedFloat(byte[] bArr, int i, float f2) {
        writeSwappedInteger(bArr, i, Float.floatToIntBits(f2));
    }
    ...
    public static void writeSwappedDouble(byte[] bArr, int i, double d2) {
        writeSwappedLong(bArr, i, Double.doubleToLongBits(d2));
    }
