...
public class ExtendedRequestRule extends AbstractRule {
    ...
    public void tag(int i, boolean z, TypeClass typeClass) {
        ...
        if (LdapTag.EXTENDED_REQUEST != ldapTagById) {
            throw new IllegalArgumentException(new StringBuffer("Expected a ExtendedRequest tag id but got a ").append(ldapTagById).toString());
        }
        getDigester().push(new ExtendedRequestImpl(getDigester().popInt()));
    }
