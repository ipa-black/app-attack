...
public class AndRule extends AbstractRule {
    ...
    public void tag(int i, boolean z, TypeClass typeClass) {
        ...
        if (tagCount < 3) {
            ...
            this.isEnabled = false;
        } else if (getDigester().getTag(2) == LdapTag.CONTEXT_SPECIFIC_TAG_4.getPrimitiveTag()) {
            this.isEnabled = false;
        } else {
    ...
    public void finish() {
        if (this.isEnabled) {
            ...
            if (getDigester().peek() == this) {
                ...
                throw new IllegalStateException(new StringBuffer("Expected to see an AndRule object on stack but got ").append(getDigester().peek().getClass()).append(" instead").toString());
            }
        }
        this.isEnabled = true;
    }
