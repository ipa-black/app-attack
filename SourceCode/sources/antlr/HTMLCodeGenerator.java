...
public class HTMLCodeGenerator extends CodeGenerator {
    ...
    public void gen(LexerGrammar lexerGrammar) throws IOException {
        ...
        println(new StringBuffer("Definition of lexer ").append(this.grammar.getClassName()).append(", which is a subclass of ").append(this.grammar.getSuperClass()).append(".").toString());
        genNextToken();
    ...
    public void gen(ParserGrammar parserGrammar) throws IOException {
        ...
        this.antlrTool.reportProgress(new StringBuffer("Generating ").append(this.grammar.getClassName()).append(".html").toString());
        this.currentOutput = this.antlrTool.openOutputFile(new StringBuffer().append(this.grammar.getClassName()).append(".html").toString());
        this.tabs = 0;
        genHeader();
        println("");
        if (this.grammar.comment != null) {
        ...
        println(new StringBuffer("Definition of parser ").append(this.grammar.getClassName()).append(", which is a subclass of ").append(this.grammar.getSuperClass()).append(".").toString());
        Enumeration elements = this.grammar.rules.elements();
        while (elements.hasMoreElements()) {
            println("");
    ...
    public void gen(RuleRefElement ruleRefElement) {
        ...
        _print(new StringBuffer("<a href=\"").append(this.grammar.getClassName()).append(".html#").append(ruleRefElement.targetRule).append("\">").toString());
    ...
    public void gen(TreeWalkerGrammar treeWalkerGrammar) throws IOException {
        ...
        println(new StringBuffer("Definition of tree parser ").append(this.grammar.getClassName()).append(", which is a subclass of ").append(this.grammar.getSuperClass()).append(".").toString());
        println("");
        this.tabs++;
        Enumeration elements = this.grammar.rules.elements();
        while (elements.hasMoreElements()) {
    ...
    protected void genGenericBlock(AlternativeBlock alternativeBlock, String str) {
        if (alternativeBlock.alternatives.size() > 1) {
            if (!this.firstElementInAlt) {
                AlternativeElement alternativeElement = this.prevAltElem;
                if (alternativeElement == null || !(alternativeElement instanceof AlternativeBlock) || ((AlternativeBlock) alternativeElement).alternatives.size() == 1) {
                    _println("");
                    print("(\t");
                } else {
                    _print("(\t");
    ...
    protected void genHeader() {
        ...
        println("<HTML>");
        println("<HEAD>");
        println(new StringBuffer("<TITLE>Grammar ").append(this.antlrTool.grammarFile).append("</TITLE>").toString());
        println("</HEAD>");
        println("<BODY>");
        println("<table summary=\"\" border=\"1\" cellpadding=\"5\">");
        println("<tr>");
        println("<td>");
        println(new StringBuffer("<font size=\"+2\">Grammar ").append(this.grammar.getClassName()).append("</font><br>").toString());
        println(new StringBuffer("<a href=\"http://www.ANTLR.org\">ANTLR</a>-generated HTML file from ").append(this.antlrTool.grammarFile).toString());
        println("<p>");
        println("Terence Parr, <a href=\"http://www.magelang.com\">MageLang Institute</a>");
        println(new StringBuffer("<br>ANTLR Version ").append(Tool.version).append("; 1989-1999").toString());
        println("</td>");
        println("</tr>");
        println("</table>");
        println("<PRE>");
    ...
    public void genRule(RuleSymbol ruleSymbol) {
        ...
        if (ruleSymbol.comment != null) {
            _println(HTMLEncode(ruleSymbol.comment));
        }
        if (ruleSymbol.access.length() != 0 && !ruleSymbol.access.equals("public")) {
            _print(new StringBuffer().append(ruleSymbol.access).append(" ").toString());
        }
        _print(new StringBuffer("<a name=\"").append(ruleSymbol.getId()).append("\">").toString());
