...
public final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                        m2010constructorimpl = SegmentOrClosed.m2010constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m2010constructorimpl = SegmentOrClosed.m2010constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m2015isClosedimpl(m2010constructorimpl)) {
                Segment m2013getSegmentimpl = SegmentOrClosed.m2013getSegmentimpl(m2010constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= m2013getSegmentimpl.getId()) {
                        break loop0;
                    } else if (!m2013getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, m2013getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m2013getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m2013getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m2013getSegmentimpl(m2010constructorimpl);
        i2 = SemaphoreKt.SEGMENT_SIZE;
        int i3 = (int) (andIncrement % i2);
        if (!Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment4.acquirers, i3, null, cancellableContinuation)) {
            symbol = SemaphoreKt.PERMIT;
            symbol2 = SemaphoreKt.TAKEN;
            if (Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment4.acquirers, i3, symbol, symbol2)) {
    ...
    private final boolean tryResumeNextFromQueue() {
            ...
            if (!SegmentOrClosed.m2015isClosedimpl(m2010constructorimpl)) {
                ...
                while (true) {
                    ...
                    if (segment.getId() >= m2013getSegmentimpl.getId()) {
                        ...
                        break;
                    } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, m2013getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m2013getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
        ...
        if (andSet == null) {
            ...
            return !Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment4.acquirers, i5, symbol3, symbol4);
        }
