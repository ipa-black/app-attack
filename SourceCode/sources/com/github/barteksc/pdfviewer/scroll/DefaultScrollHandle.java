...
public class DefaultScrollHandle extends RelativeLayout implements ScrollHandle {
    ...
    public void setScroll(float f2) {
        ...
        setPosition((this.pdfView.isSwipeVertical() ? this.pdfView.getHeight() : this.pdfView.getWidth()) * f2);
    }
    ...
    private void setPosition(float f2) {
        ...
        float f3 = width;
        float f4 = f2 - this.relativeHandlerMiddle;
        if (f4 < 0.0f) {
            f4 = 0.0f;
        } else if (f4 > f3 - Util.getDP(this.context, 40)) {
            f4 = f3 - Util.getDP(this.context, 40);
        }
        if (this.pdfView.isSwipeVertical()) {
            setY(f4);
        } else {
