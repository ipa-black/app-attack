    ...
    private static void reflectivePrintWithIndent(MessageLite messageLite, StringBuilder sb, int i) {
        ...
        for (java.lang.reflect.Method method : messageLite.getClass().getDeclaredMethods()) {
            ...
            if (method.getParameterTypes().length == 0) {
                ...
                if (method.getName().startsWith(Constants.GET)) {
                    treeSet.add(method.getName());
                }
            }
        }
        for (String str : treeSet) {
            String substring = str.startsWith(Constants.GET) ? str.substring(3) : str;
            boolean z = true;
            ...
            if (((java.lang.reflect.Method) hashMap2.get("set" + substring)) != null && (!substring.endsWith(BYTES_SUFFIX) || !hashMap.containsKey(Constants.GET + substring.substring(0, substring.length() - 5)))) {
                String str4 = substring.substring(0, 1).toLowerCase() + substring.substring(1);
                java.lang.reflect.Method method4 = (java.lang.reflect.Method) hashMap.get(Constants.GET + substring);
