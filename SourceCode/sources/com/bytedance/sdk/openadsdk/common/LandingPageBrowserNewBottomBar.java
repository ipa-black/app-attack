...
public class LandingPageBrowserNewBottomBar extends LinearLayout {
    ...
    private static ImageView Qhi(Context context, float f2, float f3, float f4, float f5) {
        PAGImageView pAGImageView = new PAGImageView(context);
        pAGImageView.setClickable(true);
        pAGImageView.setFocusable(true);
        pAGImageView.setPadding(zn.cJ(context, f4), zn.cJ(context, f5), zn.cJ(context, f4), zn.cJ(context, f5));
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(zn.cJ(context, 40.0f), zn.cJ(context, 44.0f));
        if (f2 > 0.0f) {
            layoutParams.leftMargin = zn.cJ(context, f2);
        }
        if (f3 > 0.0f) {
            layoutParams.rightMargin = zn.cJ(context, f3);
        }
