...
public class ROR implements zc, Runnable {
    ...
    private long fl;
    private final AtomicBoolean hm = new AtomicBoolean(false);
    ...
    private ROR(Activity activity, int i, Qhi qhi) {
        this.ROR = qhi;
        this.cJ = activity;
        this.CJ = i;
    }
    ...
    @Override // com.bytedance.sdk.openadsdk.utils.zc
    public void cJ() {
        if (this.fl <= 0 || this.Tgh) {
            return;
        }
        if (!this.Sf) {
            this.CJ -= SystemClock.elapsedRealtime() - this.fl;
        }
        this.Sf = true;
        if (this.CJ <= 0) {
            Tgh();
