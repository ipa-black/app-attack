...
public class hm {
    ...
    private static List<VerificationScriptResource> Qhi(Set<Gm> set) {
        ...
        for (Gm gm : set) {
            if (!TextUtils.isEmpty(gm.Qhi()) && !TextUtils.isEmpty(gm.cJ())) {
                arrayList.add(VerificationScriptResource.createVerificationScriptResourceWithParameters(gm.Qhi(), gm.ac(), gm.cJ()));
            }
            arrayList.add(VerificationScriptResource.createVerificationScriptResourceWithoutParameters(gm.ac()));
        }
        return arrayList;
    }
}
