...
public class Qhi {
    ...
    public static final cJ ac = new cJ(2);
    public static final cJ CJ = new cJ(0);
    public static final cJ fl = new cJ(1);
    public static final cJ Tgh = new cJ(2);

    public static void Qhi(cJ cJVar, boolean z, int i, long j) {
        try {
            cJVar.ROR.getAndSet(true);
            if (z) {
                cJVar.Qhi.incrementAndGet();
                cJVar.ac.addAndGet(j);
                return;
            }
            cJVar.cJ.incrementAndGet();
            Integer num = cJVar.Tgh.get(Integer.valueOf(i));
            if (num != null) {
                cJVar.Tgh.put(Integer.valueOf(i), Integer.valueOf(num.intValue() + 1));
            } else {
                cJVar.Tgh.put(Integer.valueOf(i), 1);
            }
        } catch (Throwable unused) {
        }
    }

    public static void Qhi(cJ cJVar, boolean z) {
        try {
            cJVar.ROR.getAndSet(true);
            if (z) {
                cJVar.Qhi.incrementAndGet();
