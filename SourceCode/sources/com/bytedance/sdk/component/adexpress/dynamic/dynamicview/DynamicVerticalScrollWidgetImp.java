...
public class DynamicVerticalScrollWidgetImp extends DynamicBaseWidgetImp implements Tgh {
    ...
    public void Qhi() {
        ...
        boolean z = i + 1 >= getChildCount() || ((ViewGroup) getChildAt(this.MQ + 1)).getChildCount() <= 0;
        if (!this.ABk.WAv().fl().Qhi() && z) {
            this.qMt = true;
            view = getChildAt(this.MQ - 1);
            this.Qhi = ObjectAnimator.ofFloat(childAt2, "translationY", 0.0f, (this.Sf + getChildAt(this.MQ).getHeight()) / 2);
        } else {
            if (z) {
        ...
        if (this.qMt) {
            this.cJ = ObjectAnimator.ofFloat(view, "translationY", (-(this.Sf + view.getHeight())) / 2, 0.0f);
        } else {
            this.cJ = ObjectAnimator.ofFloat(view, "translationY", (this.Sf + view.getHeight()) / 2, 0.0f);
        }
        ...
        if (this.qMt) {
            ...
            int i2 = this.MQ + 1;
            this.MQ = i2;
            this.MQ = i2 % getChildCount();
        }
        postDelayed(this.EBS, C.DEFAULT_MAX_SEEK_TO_PREVIOUS_POSITION_MS);
    }
