...
public class PdfLoader {
    private static final String SAVED_PAGE_PREFIX = "pdf_saved_page_";
    ...
    public void loadPdfFile() {
        ...
        this.sharedPreferences = Factory.getInstance().getMainNavigationActivity().getSharedPreferences(Constants.PREFS_NAME, 0);
        savedPageKey = SAVED_PAGE_PREFIX + this.initialTabId;
        if (this.pathFile.startsWith("file:///")) {
            configurator = this.pdfView.fromAsset(this.pathFile.replace("file:///android_asset/", ""));
        } else {
            if (this.pathFile.startsWith("http")) {
    ...
    public void loadConfigurator(PDFView.Configurator configurator) {
        String str = SAVED_PAGE_PREFIX + this.initialTabId;
        savedPageKey = str;
        ...
        configurator.enableSwipe(true).enableDoubletap(true).defaultPage(i).onDraw(null).onLoad(new OnLoadCompleteListener() { // from class: com.appsgeyser.multiTabApp.pdfreader.PdfLoader.4
            ...
            public void onPageChanged(int i2, int i3) {
                String unused = PdfLoader.savedPageKey = PdfLoader.SAVED_PAGE_PREFIX + Factory.getInstance().getTabsController().getSelectedTab().getWidgetInfo().getTabId();
