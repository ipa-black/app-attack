...
public class MessagingAnalytics {
    ...
    public static boolean shouldUploadScionMetrics(Intent intent) {
        ...
        return IcyHeaders.REQUEST_HEADER_ENABLE_METADATA_VALUE.equals(intent.getStringExtra(Constants.AnalyticsKeys.ENABLED));
    }
    ...
    private static void setUserPropertyIfRequired(Intent intent) {
        ...
        if (IcyHeaders.REQUEST_HEADER_ENABLE_METADATA_VALUE.equals(intent.getStringExtra(Constants.AnalyticsKeys.TRACK_CONVERSIONS))) {
    ...
    static void logToScion(String str, Intent intent) {
        ...
        if (Log.isLoggable(Constants.TAG, 3)) {
            ...
            Log.d(Constants.TAG, new StringBuilder(String.valueOf(str).length() + 37 + String.valueOf(valueOf).length()).append("Logging to scion event=").append(str).append(" scionPayload=").append(valueOf).toString());
        }
        AnalyticsConnector analyticsConnector = (AnalyticsConnector) FirebaseApp.getInstance().get(AnalyticsConnector.class);
        if (analyticsConnector == null) {
    ...
    public static int getTtl(Intent intent) {
        ...
        if (obj instanceof String) {
            try {
                ...
                Log.w(Constants.TAG, new StringBuilder(String.valueOf(valueOf).length() + 13).append("Invalid TTL: ").append(valueOf).toString());
                return 0;
            }
        }
        return 0;
    ...
    public static int getPriority(Intent intent) {
        ...
        if (stringExtra == null) {
            if (IcyHeaders.REQUEST_HEADER_ENABLE_METADATA_VALUE.equals(intent.getStringExtra(Constants.MessagePayloadKeys.PRIORITY_REDUCED_V19))) {
                return 2;
            }
