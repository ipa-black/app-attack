...
public final class Cea708Decoder extends CeaDecoder {
    ...
    private void processCurrentPacket() {
        if (this.currentDtvCcPacket.currentIndex != (this.currentDtvCcPacket.packetSize * 2) - 1) {
            Log.d(TAG, new StringBuilder(115).append("DtvCcPacket ended prematurely; size is ").append((this.currentDtvCcPacket.packetSize * 2) - 1).append(", but current index is ").append(this.currentDtvCcPacket.currentIndex).append(" (sequence number ").append(this.currentDtvCcPacket.sequenceNumber).append(");").toString());
        }
        ...
        if (readBits == 7) {
            ...
            if (readBits < 7) {
                Log.w(TAG, new StringBuilder(44).append("Invalid extended service number: ").append(readBits).toString());
            }
        }
        if (readBits2 == 0) {
            if (readBits != 0) {
                Log.w(TAG, new StringBuilder(59).append("serviceNumber is non-zero (").append(readBits).append(") when blockSize is 0").toString());
            }
        } else if (readBits != this.selectedServiceNumber) {
        } else {
            int position = this.serviceBlockPacket.getPosition() + (readBits2 * 8);
            boolean z = false;
            while (this.serviceBlockPacket.bitsLeft() > 0 && this.serviceBlockPacket.getPosition() < position) {
                ...
                if (readBits3 == 16) {
                    ...
                    if (readBits4 <= 31) {
                        ...
                        if (readBits4 <= 127) {
                            ...
                            Log.w(TAG, new StringBuilder(37).append("Invalid extended command: ").append(readBits4).toString());
                        }
                        z = true;
                    }
                    ...
                    if (readBits3 <= 127) {
                        ...
                        Log.w(TAG, new StringBuilder(33).append("Invalid base command: ").append(readBits3).toString());
                    }
                    z = true;
                }
            }
        ...
        if (i != 0) {
            if (i == 3) {
                ...
                switch (i) {
                    ...
                    default:
                        if (i >= 17 && i <= 23) {
                            Log.w(TAG, new StringBuilder(55).append("Currently unsupported COMMAND_EXT1 Command: ").append(i).toString());
                            this.serviceBlockPacket.skipBits(8);
                            return;
                        } else if (i < 24 || i > 31) {
                            Log.w(TAG, new StringBuilder(31).append("Invalid C0 command: ").append(i).toString());
                            return;
                        } else {
                            Log.w(TAG, new StringBuilder(54).append("Currently unsupported COMMAND_P16 Command: ").append(i).toString());
    ...
    private void handleC1Command(int i) {
        ...
        switch (i) {
            ...
            default:
                Log.w(TAG, new StringBuilder(31).append("Invalid C1 command: ").append(i).toString());
                return;
                ...
                default:
                    switch (i) {
                        ...
                        case 127:
                            ...
                            return;
                        default:
                            Log.w(TAG, new StringBuilder(33).append("Invalid G2 character: ").append(i).toString());
                            return;
                    }
            }
        } else {
    ...
    private void handleG3Character(int i) {
        ...
        Log.w(TAG, new StringBuilder(33).append("Invalid G3 character: ").append(i).toString());
    ...
    public static final class Cea708CueInfo {
        ...
        public Cea708CueInfo(CharSequence charSequence, Layout.Alignment alignment, float f2, int i, int i2, float f3, int i3, float f4, boolean z, int i4, int i5) {
            Cue.Builder size = new Cue.Builder().setText(charSequence).setTextAlignment(alignment).setLine(f2, i).setLineAnchor(i2).setPosition(f3).setPositionAnchor(i3).setSize(f4);
            if (z) {
