...
public final class WebvttCueParser {
        ...
        while (matcher.find()) {
            ...
            try {
                if ("line".equals(str2)) {
                    ...
                    Log.w(TAG, new StringBuilder(String.valueOf(str2).length() + 21 + String.valueOf(str3).length()).append("Unknown cue setting ").append(str2).append(":").append(str3).toString());
                } else {
    ...
    private static int parseVerticalAttribute(String str) {
        ...
        String valueOf = String.valueOf(str);
        Log.w(TAG, valueOf.length() != 0 ? "Invalid 'vertical' value: ".concat(valueOf) : new String("Invalid 'vertical' value: "));
        return Integer.MIN_VALUE;
    }
    ...
    private static void applyEntity(String str, SpannableStringBuilder spannableStringBuilder) {
        ...
        switch (c2) {
            ...
            case 3:
                ...
                return;
            default:
                Log.w(TAG, new StringBuilder(String.valueOf(str).length() + 33).append("ignoring unsupported entity: '&").append(str).append(";'").toString());
                return;
        }
    }
    ...
    public static final class WebvttCueInfoBuilder {
        ...
        private static float computeLine(float f2, int i) {
        ...
        public Cue.Builder toCueBuilder() {
            ...
            if (f2 == -3.4028235E38f) {
                f2 = derivePosition(this.textAlignment);
            }
            int i = this.positionAnchor;
            if (i == Integer.MIN_VALUE) {
                i = derivePositionAnchor(this.textAlignment);
            }
            Cue.Builder verticalType = new Cue.Builder().setTextAlignment(convertTextAlignment(this.textAlignment)).setLine(computeLine(this.line, this.lineType), this.lineType).setLineAnchor(this.lineAnchor).setPosition(f2).setPositionAnchor(i).setSize(Math.min(this.size, deriveMaxSize(i, f2))).setVerticalType(this.verticalType);
        ...
        private static Layout.Alignment convertTextAlignment(int i) {
            if (i != 1) {
                ...
                if (i != 3) {
                    if (i != 4) {
                        if (i != 5) {
                            Log.w(WebvttCueParser.TAG, new StringBuilder(34).append("Unknown textAlignment: ").append(i).toString());
                            return null;
                        }
                    }
                }
        ...
        private static float deriveMaxSize(int i, float f2) {
            if (i != 0) {
                if (i == 1) {
                    return f2 <= WebvttCueParser.DEFAULT_POSITION ? f2 * 2.0f : (1.0f - f2) * 2.0f;
                } else if (i == 2) {
                    return f2;
                } else {
