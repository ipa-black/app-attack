...
final class WebvttCssParser {
    ...
    static String parseNextToken(ParsableByteArray parsableByteArray, StringBuilder sb) {
        ...
        if ("".equals(parseIdentifier)) {
            return new StringBuilder(1).append((char) parsableByteArray.readUnsignedByte()).toString();
        }
    ...
    private static void parseFontSize(String str, WebvttCssStyle webvttCssStyle) {
        ...
        if (!matcher.matches()) {
            Log.w(TAG, new StringBuilder(String.valueOf(str).length() + 22).append("Invalid font-size: '").append(str).append("'.").toString());
            return;
        }
