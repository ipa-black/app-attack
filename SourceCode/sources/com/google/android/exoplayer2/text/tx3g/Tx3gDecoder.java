...
public final class Tx3gDecoder extends SimpleSubtitleDecoder {
    ...
    private void applyStyleRecord(ParsableByteArray parsableByteArray, SpannableStringBuilder spannableStringBuilder) throws SubtitleDecoderException {
        ...
        if (readUnsignedShort2 > spannableStringBuilder.length()) {
            Log.w(TAG, new StringBuilder(68).append("Truncating styl end (").append(readUnsignedShort2).append(") to cueText.length() (").append(spannableStringBuilder.length()).append(").").toString());
            readUnsignedShort2 = spannableStringBuilder.length();
        }
        if (readUnsignedShort >= readUnsignedShort2) {
            Log.w(TAG, new StringBuilder(60).append("Ignoring styl with start (").append(readUnsignedShort).append(") >= end (").append(readUnsignedShort2).append(").").toString());
            return;
        }
