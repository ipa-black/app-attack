...
public final class HlsMediaPeriod implements MediaPeriod, HlsSampleStreamWrapper.Callback, HlsPlaylistTracker.PlaylistEventListener {
    ...
    private void buildAndPrepareSampleStreamWrappers(long j) {
        ...
        while (i3 < list2.size()) {
            ...
            String sb = new StringBuilder(String.valueOf(str).length() + 21).append("subtitle:").append(i3).append(":").append(str).toString();
            Uri[] uriArr = new Uri[i];
            uriArr[i2] = rendition.url;
            Format[] formatArr = new Format[i];
            formatArr[i2] = rendition.format;
            ArrayList arrayList3 = arrayList2;
            int i4 = i3;
            HlsSampleStreamWrapper buildSampleStreamWrapper = buildSampleStreamWrapper(sb, 3, uriArr, formatArr, null, Collections.emptyList(), map, j);
        ...
        if (this.allowChunklessPreparation && z3) {
            ...
            if (codecCountOfType > 0) {
                ...
                if (codecCountOfType2 > 0 && (hlsMultivariantPlaylist.muxedAudioFormat != null || hlsMultivariantPlaylist.audios.isEmpty())) {
                    arrayList.add(new TrackGroup("main".concat(":audio"), deriveAudioFormat(formatArr[0], hlsMultivariantPlaylist.muxedAudioFormat, false)));
                }
                List<Format> list3 = hlsMultivariantPlaylist.muxedCaptionFormats;
                if (list3 != null) {
                    for (int i7 = 0; i7 < list3.size(); i7++) {
                        arrayList.add(new TrackGroup(new StringBuilder("main".length() + 15).append("main:cc:").append(i7).toString(), list3.get(i7)));
                    }
                }
            } else {
            ...
            TrackGroup trackGroup = new TrackGroup("main".concat(":id3"), new Format.Builder().setId("ID3").setSampleMimeType(MimeTypes.APPLICATION_ID3).build());
    ...
    private void buildAndPrepareAudioSampleStreamWrappers(long j, List<HlsMultivariantPlaylist.Rendition> list, List<HlsSampleStreamWrapper> list2, List<int[]> list3, Map<String, DrmInitData> map) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (hashSet.add(str)) {
                ...
                HlsSampleStreamWrapper buildSampleStreamWrapper = buildSampleStreamWrapper(concat, 1, (Uri[]) arrayList.toArray((Uri[]) Util.castNonNullTypeArray(new Uri[0])), (Format[]) arrayList2.toArray(new Format[0]), null, Collections.emptyList(), map, j);
                list3.add(Ints.toArray(arrayList3));
                list2.add(buildSampleStreamWrapper);
                if (this.allowChunklessPreparation && z) {
                    buildSampleStreamWrapper.prepareWithMultivariantPlaylistInfo(new TrackGroup[]{new TrackGroup(concat, (Format[]) arrayList2.toArray(new Format[0]))}, 0, new int[0]);
                }
            }
        }
    }
