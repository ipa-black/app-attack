...
final class WavHeaderReader {
    ...
    public static boolean checkFileType(ExtractorInput extractorInput) throws IOException {
        ...
        if (peek.id == 1380533830 || peek.id == 1380333108) {
            ...
            if (readInt != 1463899717) {
                Log.e(TAG, new StringBuilder(34).append("Unsupported form type: ").append(readInt).toString());
                return false;
            }
            return true;
        }
    ...
    private static ChunkHeader skipToChunk(int i, ExtractorInput extractorInput, ParsableByteArray parsableByteArray) throws IOException {
        ChunkHeader peek = ChunkHeader.peek(extractorInput, parsableByteArray);
        while (peek.id != i) {
            Log.w(TAG, new StringBuilder(39).append("Ignoring unknown WAV chunk: ").append(peek.id).toString());
            long j = peek.size + 8;
            if (j > 2147483647L) {
                throw ParserException.createForUnsupportedContainerFeature(new StringBuilder(51).append("Chunk is too large (~2GB+) to skip; id: ").append(peek.id).toString());
            }
