...
public final class zzfvk {
    ...
    public static PendingIntent zza(Context context, int i, Intent intent, int i2, int i3) {
        zzfye.zzf((i2 & 95) == 0, "Cannot set any dangerous parts of intent to be mutable.");
        zzfye.zzf(intent.getComponent() != null, "Must set component on Intent.");
        if (zzb(0, 1)) {
            zzfye.zzf(!zzb(i2, 67108864), "Cannot set mutability flags if PendingIntent.FLAG_IMMUTABLE is set.");
        } else {
            zzfye.zzf(zzb(i2, 67108864), "Must set PendingIntent.FLAG_IMMUTABLE for SDK >= 23 if no parts of intent are mutable.");
        }
