package com.google.android.gms.internal.ads;
/* compiled from: com.google.android.gms:play-services-ads@@21.5.0 */
/* loaded from: classes4.dex */
public final class zzgto extends zzgyd implements zzgzo {
    private static final zzgto zzb;
    private int zzd;
    private zzgym zze = zzaL();

    static {
        zzgto zzgtoVar = new zzgto();
        zzb = zzgtoVar;
        zzgyd.zzaS(zzgto.class, zzgtoVar);
    }

    private zzgto() {
    }

    public static zzgtl zza() {
        return (zzgtl) zzb.zzaz();
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static /* synthetic */ void zze(zzgto zzgtoVar, zzgtn zzgtnVar) {
        zzgtnVar.getClass();
        zzgym zzgymVar = zzgtoVar.zze;
        if (!zzgymVar.zzc()) {
            zzgtoVar.zze = zzgyd.zzaM(zzgymVar);
        }
        zzgtoVar.zze.add(zzgtnVar);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    @Override // com.google.android.gms.internal.ads.zzgyd
    public final Object zzb(int i, Object obj, Object obj2) {
        int i2 = i - 1;
        if (i2 != 0) {
            if (i2 == 2) {
                return zzaP(zzb, "\u0000\u0002\u0000\u0000\u0001\u0002\u0002\u0000\u0001\u0000\u0001\u000b\u0002\u001b", new Object[]{"zzd", "zze", zzgtn.class});
            } else if (i2 != 3) {
                if (i2 != 4) {
                    if (i2 != 5) {
                        return null;
                    }
                    return zzb;
                }
                return new zzgtl(null);
            } else {
                return new zzgto();
            }
        }
        return (byte) 1;
    }
}
