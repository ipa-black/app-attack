...
public final class zzgvc {
    ...
    public static KeyPair zzc(ECParameterSpec eCParameterSpec) throws GeneralSecurityException {
        KeyPairGenerator keyPairGenerator = (KeyPairGenerator) zzgve.zzf.zza("EC");
    ...
    public static byte[] zzg(ECPrivateKey eCPrivateKey, ECPublicKey eCPublicKey) throws GeneralSecurityException {
        ...
        PublicKey generatePublic = ((KeyFactory) zzgve.zzg.zza("EC")).generatePublic(new ECPublicKeySpec(w, eCPrivateKey.getParams()));
        KeyAgreement keyAgreement = (KeyAgreement) zzgve.zze.zza("ECDH");
        ...
        try {
            ...
            if (bigInteger3.signum() == -1 || bigInteger3.compareTo(zzb(curve)) >= 0) {
                throw new GeneralSecurityException("shared secret is out of range");
            }
            BigInteger zzb = zzb(curve);
            BigInteger mod = bigInteger3.multiply(bigInteger3).add(curve.getA()).multiply(bigInteger3).add(curve.getB()).mod(zzb);
            if (zzb.signum() != 1) {
            ...
            if (mod2.equals(BigInteger.ZERO)) {
                ...
                if (bigInteger != null && bigInteger.multiply(bigInteger).mod(zzb).compareTo(mod2) != 0) {
    ...
    public static ECPrivateKey zzh(int i, byte[] bArr) throws GeneralSecurityException {
        return (ECPrivateKey) ((KeyFactory) zzgve.zzg.zza("EC")).generatePrivate(new ECPrivateKeySpec(new BigInteger(1, bArr), zzk(i)));
    }
    ...
    public static ECPublicKey zzi(int i, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ...
        return (ECPublicKey) ((KeyFactory) zzgve.zzg.zza("EC")).generatePublic(new ECPublicKeySpec(eCPoint, zzk));
    }
    ...
    public static ECPublicKey zzj(ECParameterSpec eCParameterSpec, int i, byte[] bArr) throws GeneralSecurityException {
        ...
        zzd(eCPoint, curve);
        return (ECPublicKey) ((KeyFactory) zzgve.zzg.zza("EC")).generatePublic(new ECPublicKeySpec(eCPoint, eCParameterSpec));
    }

    public static byte[] zzl(int i, int i2, ECPoint eCPoint) throws GeneralSecurityException {
        return zzm(zzk(i).getCurve(), 1, eCPoint);
    }

    public static byte[] zzm(EllipticCurve ellipticCurve, int i, ECPoint eCPoint) throws GeneralSecurityException {
