...
abstract class zzgjb {
    private final zzgiz zza;
    private final zzgiz zzb;

    public zzgjb(byte[] bArr) throws GeneralSecurityException {
        if (!zzgjh.zza(1)) {
    ...
    abstract zzgiz zza(byte[] bArr, int i) throws InvalidKeyException;
    ...
    public final void zzb(ByteBuffer byteBuffer, byte[] bArr, byte[] bArr2, byte[] bArr3) throws GeneralSecurityException {
        ...
        byte[] zza = zzgje.zza(zze(bArr), zzf(bArr3, byteBuffer));
        byteBuffer.limit(byteBuffer.limit() + 16);
        byteBuffer.put(zza);
    }
    ...
    public final byte[] zzc(ByteBuffer byteBuffer, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ...
        try {
            if (MessageDigest.isEqual(zzgje.zza(zze(bArr), zzf(bArr2, byteBuffer)), bArr3)) {
                byteBuffer.position(position);
