...
public final class ClosingFuture<V> {
    ...
    public static void closeQuietly(@CheckForNull final AutoCloseable autoCloseable, Executor executor) {
        ...
        try {
            executor.execute(new Runnable() { // from class: com.google.common.util.concurrent.ClosingFuture.11
                ...
                public void run() {
                    try {
                        ...
                        ClosingFuture.logger.log(Level.WARNING, "thrown by close()", (Throwable) e2);
                    }
                }
            });
            ...
            if (logger2.isLoggable(Level.WARNING)) {
                logger2.log(Level.WARNING, String.format("while submitting close to %s; will close inline", executor), (Throwable) e2);
            }
    ...
    private boolean compareAndUpdateState(State state, State state2) {
        return b$$ExternalSyntheticBackportWithForwarding0.m(this.state, state, state2);
    }
