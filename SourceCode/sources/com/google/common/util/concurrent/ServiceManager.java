...
public final class ServiceManager implements ServiceManagerBridge {
    ...
    public ServiceManager startAsync() {
        ...
        while (it2.hasNext()) {
            ...
            try {
                ...
                logger2.log(level, new StringBuilder(String.valueOf(valueOf).length() + 24).append("Unable to start Service ").append(valueOf).toString(), (Throwable) e2);
            }
        }
        return this;
    }
    ...
    public ImmutableMap<Service, Duration> startupDurations() {
        return ImmutableMap.copyOf(Maps.transformValues(startupTimes(), new Function() { // from class: com.google.common.util.concurrent.ServiceManager$$ExternalSyntheticLambda0
    ...
    public static final class ServiceManagerState {
        ...
        void checkHealthy() {
            if (this.states.count(Service.State.RUNNING) != this.numberOfServices) {
                ...
                IllegalStateException illegalStateException = new IllegalStateException(new StringBuilder(String.valueOf(valueOf).length() + 79).append("Expected to be healthy after starting. The following services are not running: ").append(valueOf).toString());
