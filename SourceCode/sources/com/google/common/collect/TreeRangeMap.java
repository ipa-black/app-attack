...
public final class TreeRangeMap<K extends Comparable, V> implements RangeMap<K, V> {
    private static final RangeMap<Comparable<?>, Object> EMPTY_SUB_RANGE_MAP = new RangeMap<Comparable<?>, Object>() { // from class: com.google.common.collect.TreeRangeMap.1
        ...
        public void merge(Range<Comparable<?>> range, @CheckForNull Object obj, BiFunction<? super Object, ? super Object, ? extends Object> biFunction) {
            Preconditions.checkNotNull(range);
            String valueOf = String.valueOf(range);
            throw new IllegalArgumentException(new StringBuilder(String.valueOf(valueOf).length() + 45).append("Cannot merge range ").append(valueOf).append(" into an empty subRangeMap").toString());
        }
    ...
    public void merge(Range<K> range, @CheckForNull V v, BiFunction<? super V, ? super V, ? extends V> biFunction) {
        ...
        if (v != null) {
            ...
            for (Map.Entry<Cut<K>, RangeMapEntry<K, V>> entry : entrySet) {
                RangeMapEntry<K, V> value = entry.getValue();
        ...
        while (it.hasNext()) {
            Map.Entry<Cut<K>, RangeMapEntry<K, V>> next = it.next();
            V apply = biFunction.apply((V) next.getValue().getValue(), v);
            if (apply == null) {
                it.remove();
            } else {
