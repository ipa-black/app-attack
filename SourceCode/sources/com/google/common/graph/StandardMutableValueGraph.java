...
public final class StandardMutableValueGraph<N, V> extends StandardValueGraph<N, V> implements MutableValueGraph<N, V> {
    ...
    public boolean removeNode(N n) {
        ...
        for (N n2 : graphConnections.successors()) {
            ((GraphConnections) Objects.requireNonNull(this.nodeConnections.getWithoutCaching(n2))).removePredecessor(n);
            this.edgeCount--;
        }
        if (isDirected()) {
            for (N n3 : graphConnections.predecessors()) {
                Preconditions.checkState(((GraphConnections) Objects.requireNonNull(this.nodeConnections.getWithoutCaching(n3))).removeSuccessor(n) != null);
                this.edgeCount--;
            }
        }
