...
class HttpProxyCache extends ProxyCache {
    ...
    public void processRequest(GetRequest getRequest, Socket socket) throws IOException, ProxyCacheException {
        ...
        bufferedOutputStream.write(newResponseHeaders(getRequest).getBytes(C.UTF8_NAME));
    ...
    private String newResponseHeaders(GetRequest getRequest) throws IOException, ProxyCacheException {
        String mime;
        boolean z = true;
        boolean z2 = !TextUtils.isEmpty(this.source.getMime());
        ...
        if (!z3 || !getRequest.partial) {
            z = false;
        }
        return (getRequest.partial ? "HTTP/1.1 206 PARTIAL CONTENT\n" : "HTTP/1.1 200 OK\n") + "Accept-Ranges: bytes\n" + (z3 ? format("Content-Length: %d\n", Long.valueOf(j)) : "") + (z ? format("Content-Range: bytes %d-%d/%d\n", Long.valueOf(getRequest.rangeOffset), Long.valueOf(available - 1), Long.valueOf(available)) : "") + (z2 ? format("Content-Type: %s\n", mime) : "") + "\n";
    }
