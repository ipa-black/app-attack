...
public final class JavaTypeResolver {
    ...
    private final LazyJavaResolverContext f16379c;
    ...
    public final KotlinType transformJavaType(JavaType javaType, JavaTypeAttributes attr) {
        ...
        if (javaType instanceof JavaPrimitiveType) {
            ...
            if (type != null) {
                unitType = this.f16379c.getModule().getBuiltIns().getPrimitiveKotlinType(type);
            } else {
                unitType = this.f16379c.getModule().getBuiltIns().getUnitType();
            }
            ...
            if (!(javaType instanceof JavaWildcardType)) {
                if (javaType == null) {
                    SimpleType defaultBound = this.f16379c.getModule().getBuiltIns().getDefaultBound();
            ...
            if (bound == null) {
                SimpleType defaultBound2 = this.f16379c.getModule().getBuiltIns().getDefaultBound();
    ...
    public final KotlinType transformArrayType(JavaArrayType arrayType, JavaTypeAttributes attr, boolean z) {
        ...
        LazyJavaAnnotations lazyJavaAnnotations = new LazyJavaAnnotations(this.f16379c, arrayType, true);
        if (type != null) {
            SimpleType primitiveArrayKotlinType = this.f16379c.getModule().getBuiltIns().getPrimitiveArrayKotlinType(type);
        ...
        if (attr.isForAnnotationParameter()) {
            SimpleType arrayType2 = this.f16379c.getModule().getBuiltIns().getArrayType(z ? Variance.OUT_VARIANCE : Variance.INVARIANT, transformJavaType, lazyJavaAnnotations);
        ...
        SimpleType arrayType3 = this.f16379c.getModule().getBuiltIns().getArrayType(Variance.INVARIANT, transformJavaType, lazyJavaAnnotations2);
        Intrinsics.checkNotNullExpressionValue(arrayType3, "c.module.builtIns.getArr…mponentType, annotations)");
        return KotlinTypeFactory.flexibleType(arrayType3, this.f16379c.getModule().getBuiltIns().getArrayType(Variance.OUT_VARIANCE, transformJavaType, lazyJavaAnnotations2).makeNullableAsSpecified(true));
    }
    ...
    private final SimpleType computeSimpleJavaClassifierType(JavaClassifierType javaClassifierType, JavaTypeAttributes javaTypeAttributes, SimpleType simpleType) {
        LazyJavaAnnotations lazyJavaAnnotations = simpleType == null ? new LazyJavaAnnotations(this.f16379c, javaClassifierType, false, 4, null) : simpleType.getAnnotations();
    ...
    private final TypeConstructor computeTypeConstructor(JavaClassifierType javaClassifierType, JavaTypeAttributes javaTypeAttributes) {
        ...
        if (classifier instanceof JavaClass) {
            ...
            if (mapKotlinClass == null) {
                mapKotlinClass = this.f16379c.getComponents().getModuleClassResolver().resolveClass(javaClass);
            }
    ...
    private final TypeConstructor createNotFoundClass(JavaClassifierType javaClassifierType) {
        ...
        TypeConstructor typeConstructor = this.f16379c.getComponents().getDeserializedDescriptorResolver().getComponents().getNotFoundClasses().getClass(classId, CollectionsKt.listOf(0)).getTypeConstructor();
    ...
    private final ClassDescriptor mapKotlinClass(JavaClassifierType javaClassifierType, JavaTypeAttributes javaTypeAttributes, FqName fqName) {
        if (javaTypeAttributes.isForAnnotationParameter() && Intrinsics.areEqual(fqName, JavaTypeResolverKt.access$getJAVA_LANG_CLASS_FQ_NAME$p())) {
            return this.f16379c.getComponents().getReflectionTypes().getKClass();
        }
        JavaToKotlinClassMapper javaToKotlinClassMapper = JavaToKotlinClassMapper.INSTANCE;
        ClassDescriptor mapJavaToKotlin$default = JavaToKotlinClassMapper.mapJavaToKotlin$default(javaToKotlinClassMapper, fqName, this.f16379c.getModule().getBuiltIns(), null, 4, null);
    ...
    private final List<TypeProjection> computeRawTypeArguments(final JavaClassifierType javaClassifierType, List<? extends TypeParameterDescriptor> list, final TypeConstructor typeConstructor, final JavaTypeAttributes javaTypeAttributes) {
        ...
        for (final TypeParameterDescriptor typeParameterDescriptor : list2) {
            if (TypeUtilsKt.hasTypeParameterRecursiveBounds(typeParameterDescriptor, null, javaTypeAttributes.getVisitedTypeParameters())) {
                ...
                computeProjection = this.rawSubstitution.computeProjection(typeParameterDescriptor, javaClassifierType.isRaw() ? javaTypeAttributes : javaTypeAttributes.withFlexibility(JavaTypeFlexibility.INFLEXIBLE), new LazyWrappedType(this.f16379c.getStorageManager(), new Function0<KotlinType>() { // from class: kotlin.reflect.jvm.internal.impl.load.java.lazy.types.JavaTypeResolver$computeRawTypeArguments$1$erasedUpperBound$1
                    ...
                    public final KotlinType invoke() {
                        ...
                        KotlinType erasedUpperBound$descriptors_jvm = typeParameterUpperBoundEraser.getErasedUpperBound$descriptors_jvm(typeParameterDescriptor2, isRaw, javaTypeAttributes2.withDefaultType(mo1763getDeclarationDescriptor == null ? null : mo1763getDeclarationDescriptor.getDefaultType()));
