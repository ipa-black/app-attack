                ...
                if (supertypesSet.add(current)) {
                    ...
                    if (none == null) {
                        ...
                        for (KotlinTypeMarker kotlinTypeMarker : typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current))) {
                            SimpleTypeMarker mo1769transformType = none.mo1769transformType(typeCheckerState, kotlinTypeMarker);
                            if ((typeSystemContext.isClassType(mo1769transformType) && !typeSystemContext.isMarkedNullable(mo1769transformType)) || typeSystemContext.isDefinitelyNotNullType(mo1769transformType)) {
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    for (KotlinTypeMarker kotlinTypeMarker : typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current))) {
                        SimpleTypeMarker mo1769transformType = supertypesPolicy.mo1769transformType(state, kotlinTypeMarker);
                        if (INSTANCE.isApplicableAsEndNode(state, mo1769transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(mo1769transformType);
